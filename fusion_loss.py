...
class fusionloss(nn.Module):
    ...
    def forward(self,vi_en_y,vi_en,ir,y_f,I_f,weight_image,weight_grad,weight_color):
        ...
        total_loss = weight_image * image_loss + weight_grad * grad_loss + weight_color * color_loss
...
class CosineColorLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.l2_normalize = F.normalize

    def forward(self, image_fused_rgb, image_vis_rgb):

        image_fused_norm = self.l2_normalize(image_fused_rgb, p=2, dim=1)
        image_vis_norm = self.l2_normalize(image_vis_rgb, p=2, dim=1)


        cosine_similarity = torch.sum(image_fused_norm * image_vis_norm, dim=1)


        loss = torch.mean(1.0 - cosine_similarity)
        return loss

class fusionloss_1(nn.Module):
    def __init__(self):
        super(fusionloss_1, self).__init__()
        self.sobelconv = Sobelxy()
        self.mse_loss = nn.MSELoss()
        self.fuionnet = model.FusionNet().cuda()
        self.angle = angle()
        ...
        self.color_loss_calculator = CosineColorLoss()


    def forward(self, vi_en_y, vi_en, ir, y_f, I_f, weight_color,weight_grad,weight_image):
        vi_en_y_gard = self.sobelconv(vi_en_y)
        ir_gard = self.sobelconv(ir)
        y_f_grad = self.sobelconv(y_f)
        max_grad = torch.max(vi_en_y_gard, ir_gard)
        ...
        color_loss = self.color_loss_calculator(I_f, vi_en)
...
def cc(img1, img2):
    ...
    denominator = (eps + torch.sqrt(torch.sum(img1 ** 2, dim=-1)) * torch.sqrt(torch.sum(img2 ** 2, dim=-1)))
