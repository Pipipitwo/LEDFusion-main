...
class Conv2d_cd(nn.Module):
    ...
    def get_weight(self):
        ...
        # --- FIX: Replaced hardcoded torch.cuda.FloatTensor ---
        # Get the correct device from the existing weight tensor and create the new tensor there.
...
class Conv2d_rd(nn.Module):
    ...
    def forward(self, x):
        ...
        if math.fabs(self.theta - 0.0) < 1e-8:
            ...
            # --- FIX: Replaced brittle if/else block with a single device-agnostic line ---
            device = self.conv.weight.device
            conv_weight_rd = torch.zeros(conv_shape[0], conv_shape[1], 5 * 5, device=device)
